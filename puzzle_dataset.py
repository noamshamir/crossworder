    ...
    def _lazy_load_dataset(self):
        ...
        for set_name in self.metadata.sets: # Load subset
            for i, dataset_path in enumerate(self.config.dataset_paths):
                ...
                # Optionally load clue_embeddings (used by crossword datasets)
                # These are stored per-group (not per-example) to save space
                clue_emb_path = os.path.join(dataset_path, self.split, f"{set_name}__clue_embeddings.npy")
                if os.path.exists(clue_emb_path):
                    self._data[set_name_]["clue_embeddings"] = np.load(clue_emb_path, mmap_mode="r")
                    self._data[set_name_]["_has_group_clue_embeddings"] = True


    def _lookup_clue_embeddings(self, dataset, example_indices):
        """Look up clue embeddings by mapping example indices → group indices.
        
        Clue embeddings are stored per-group (one per original puzzle) to save
        space.  We use group_indices to find which group each example belongs to.
        """
        group_indices = dataset["group_indices"]
        # searchsorted: find the group that each example index falls into
        group_ids = np.searchsorted(group_indices[1:], example_indices, side="right")
        return dataset["clue_embeddings"][group_ids]

    def _collate_batch(self, batch):
        # Convert dtype (float fields stay float, int fields become int32)
        batch = {
            k: (v.astype(np.float32) if np.issubdtype(v.dtype, np.floating) else v.astype(np.int32))
        ...
        if batch["puzzle_identifiers"].size < self.local_batch_size:
            ...
            # Default pad value is 0 for any extra fields (e.g. clue_embeddings)
            for k in batch:
                if k not in pad_values:
                    pad_values[k] = 0
    ...
    def _iter_test(self):
        for set_i, (set_name, dataset) in enumerate(self._data.items()):  # type: ignore
            ...
            while start_index < total_examples:
                ...
                if dataset.get("_has_group_clue_embeddings"):
                    batch_dict["clue_embeddings"] = self._lookup_clue_embeddings(
                        dataset, np.arange(local_start, local_end)
                    )
